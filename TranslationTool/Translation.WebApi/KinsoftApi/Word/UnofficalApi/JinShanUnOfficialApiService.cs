    ...
    public class KinsoftUnOfficialApiService : WebRequestBase
    {
        public static async Task<EnglishWordTranslationData> GetWordsAsync(string queryText)
        {
            ...
            try
            {
                ...
                var youDaoResponse = JsonConvert.DeserializeObject<KinsoftIntergateDataResponse>(result);
                var translationData = new EnglishWordTranslationData()
                {
                    Word = youDaoResponse.KinsoftBaseInfoDictionary?.WordName ?? string.Empty,
                    DetailJson = result,
                };
                translationData.UkPronounce = youDaoResponse.KinsoftBaseInfoDictionary?.GetUkPronounce() ?? new PronounceInfo();
                translationData.UsPronounce = youDaoResponse.KinsoftBaseInfoDictionary?.GetUsPronounce() ?? new PronounceInfo();
                translationData.Translations = youDaoResponse.KinsoftBaseInfoDictionary?.GetTranslations() ?? new List<SematicInfo>();

                var phraseInfos = new List<PhraseInfo>();
                youDaoResponse.KinsoftPhraseInfos?.ForEach(i => phraseInfos.AddRange(i.GetPhrases()));
                translationData.Phrases = phraseInfos;

                var synonymInfos = new List<SynonymInfo>();
                youDaoResponse.KinsoftSynonymInfos?.ForEach(i => synonymInfos.AddRange(i.GetSynonymWords()));
                translationData.Synonyms = synonymInfos;

                var cognates = new List<CognateInfo>();
                youDaoResponse.KinsoftNetMeanInfoDictionary?.GetCognateWords().ForEach(i => cognates.Add(i));
                translationData.Cognates = cognates;

                var sentences = new List<SentenceInfo>();
                youDaoResponse.KinsoftSentences?.ForEach(i => sentences.Add(new SentenceInfo()
                {
